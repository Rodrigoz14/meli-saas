...
const TEMPLATE_RENDIMIENTO = `Usa la imagen adjunta del producto EXACTAMENTE como está, sin modificarla. Solo se compone sobre el fondo nuevo.

Genera una infografía cuadrada de 1200x1200 px para Mercado Libre que destaque cuánto rinde/dura {NOMBRE_PRODUCTO}.

COMPOSICIÓN:
- Número gigante como protagonista absoluto, tipografía extra bold, en {COLOR_ACENTO}, con una etiqueta corta arriba o al lado en {COLOR_TEXTO}: "{FRASE_GANCHO_LINEA_1}".
- Badge circular o de cinta en {COLOR_ACENTO} con el mismo dato, repetido como refuerzo visual (redundancia intencional, como un sello).
- El producto (imagen subida) al costado, tamaño mediano (40-50% del alto), sombra realista.
- Fondo en {COLOR_PRIMARIO} con 1-2 formas geométricas desenfocadas en {COLOR_ACENTO} de baja opacidad, y unas líneas diagonales cortas tipo "destello" cerca del número para darle energía.

BARRA INFERIOR: 2-3 datos cortos con ícono, mismo estilo lineal, texto en {COLOR_TEXTO}: "{BENEFICIO_1}", "{BENEFICIO_2}", "{BENEFICIO_3}".

ESTILO: sigue el SISTEMA DE DISEÑO. El número debe ser el elemento más grande de toda la pieza.

NO HACER: no inventar una cifra de rendimiento que no esté en el texto — si no hay número real, usa la frase genérica sin cifra. No usar fondo negro ni oscuro.`;

const TEMPLATE_INSTRUCCIONES = `Usa la imagen adjunta del producto EXACTAMENTE como está, sin modificarla. Solo se compone sobre el fondo nuevo.

Genera una infografía cuadrada de 1200x1200 px para Mercado Libre que explique el modo de uso/dosis de {NOMBRE_PRODUCTO}.

COMPOSICIÓN:
- Título superior extra bold en 2 líneas: línea 1 en {COLOR_TEXTO} "{FRASE_GANCHO_LINEA_1}", línea 2 en {COLOR_ACENTO} más grande "{FRASE_GANCHO_LINEA_2}".
- Un ícono grande de reloj/calendario/medida (según corresponda) en {COLOR_ACENTO}, junto al texto de la dosis en tipografía muy grande.
- El producto (imagen subida) al costado, tamaño mediano (40-50% del alto), sombra realista, apoyado sobre una superficie sutil.
- Fondo en {COLOR_PRIMARIO} con una banda o forma geométrica en {COLOR_ACENTO} de baja opacidad detrás del ícono principal.

BLOQUE INFERIOR: 2-3 tarjetas en {COLOR_PANEL} con esquinas redondeadas, ícono + texto en {COLOR_TEXTO}: "{BENEFICIO_1}", "{BENEFICIO_2}", "{BENEFICIO_3}".

ESTILO: sigue el SISTEMA DE DISEÑO. Tono claro e instructivo, como una guía rápida.

NO HACER: no inventar una dosis, frecuencia o modo de uso que no esté en el texto real. No usar fondo negro ni oscuro.`;

const TEMPLATE_CONFIANZA = `Usa la imagen adjunta del producto EXACTAMENTE como está, sin modificarla. Solo se compone sobre el fondo nuevo.

Genera una infografía cuadrada de 1200x1200 px para Mercado Libre enfocada en transmitir confianza/origen de {NOMBRE_PRODUCTO}.

COMPOSICIÓN:
- Título superior extra bold en 2 líneas: línea 1 en {COLOR_TEXTO} "{FRASE_GANCHO_LINEA_1}", línea 2 en {COLOR_ACENTO} más grande "{FRASE_GANCHO_LINEA_2}".
- Un emblema o sello circular grande en {COLOR_ACENTO} (estilo insignia/medalla, sin imitar logos oficiales de certificación reales) como elemento gráfico principal junto al título.
- El producto (imagen subida) al costado, tamaño mediano (40-50% del alto), sombra realista.
- Fondo en {COLOR_PRIMARIO} con formas geométricas desenfocadas en {COLOR_ACENTO} y una textura sutil de líneas finas, transmitiendo solidez.

BARRA INFERIOR: 2-3 claims de respaldo con ícono, mismo estilo lineal, texto en {COLOR_TEXTO}: "{BENEFICIO_1}", "{BENEFICIO_2}", "{BENEFICIO_3}".

ESTILO: sigue el SISTEMA DE DISEÑO. Tono sobrio y confiable, sin exagerar.

NO HACER: no inventar ni representar certificaciones, sellos oficiales, premios o normas reales que no estén en el texto — el emblema es decorativo, no debe parecer una certificación real. No usar fondo negro ni oscuro.`;

const TEMPLATE_PRODUCTO_LIMPIO = `Usa la imagen adjunta del producto EXACTAMENTE como está: no cambies su forma, etiqueta, texto, colores, proporciones ni ángulo. No la reinterpretes ni la regeneres. Trátala como un recorte fijo que debes insertar sobre un fondo nuevo.

Genera una infografía cuadrada de 1200x1200 px para Mercado Libre tipo "foto de producto" minimalista — casi sin texto, el protagonista absoluto es el envase.

COMPOSICIÓN:
- El producto ({NOMBRE_PRODUCTO}) centrado, ocupando 65-75% de la altura del lienzo, apoyado sobre un pedestal o superficie con reflejo sutil degradado hacia abajo, sombra de contacto realista.
- Fondo minimalista en {COLOR_PRIMARIO} con un degradado radial suave hacia {COLOR_ACENTO} y una sola forma geométrica grande y muy desenfocada detrás del producto, sin más elementos.
- Como único texto (opcional, chico, discreto, en una esquina inferior): "{FRASE_GANCHO_LINEA_1}" en {COLOR_TEXTO}.

ESTILO: sigue el SISTEMA DE DISEÑO. Máxima limpieza visual, como una foto de catálogo premium — el fondo debe seguir teniendo la textura/forma sutil del sistema de diseño, pero sin ningún bloque de texto grande, bullets ni íconos.

NO HACER: no agregar bullets, tarjetas ni íconos — esta es la única pieza del set sin bloques de venta. No usar fondo negro ni oscuro.`;

const TEMPLATE_COMPOSICION = `Usa la imagen adjunta del producto EXACTAMENTE como está, sin modificarla. Solo se compone sobre el fondo nuevo.

Genera una infografía cuadrada de 1200x1200 px para Mercado Libre que desglose los ingredientes/componentes reales de {NOMBRE_PRODUCTO}.

COMPOSICIÓN:
- Título superior extra bold: "{FRASE_GANCHO_LINEA_1}" en {COLOR_TEXTO}, con "{FRASE_GANCHO_LINEA_2}" en {COLOR_ACENTO} debajo.
- 3-4 esferas o íconos circulares grandes, mismo estilo, uno por ingrediente/componente, cada uno con una etiqueta corta debajo en {COLOR_TEXTO}: "{BENEFICIO_1}", "{BENEFICIO_2}", "{BENEFICIO_3}", "{BENEFICIO_4}".
- El producto (imagen subida) al costado o abajo, tamaño mediano (35-45% del alto).
- Fondo en {COLOR_PRIMARIO} con formas geométricas desenfocadas en {COLOR_ACENTO}.

ESTILO: sigue el SISTEMA DE DISEÑO. Look técnico/informativo pero limpio, como una ficha nutricional ilustrada.

NO HACER: no inventar ingredientes, cantidades ni compuestos que no estén en el texto real. No usar fondo negro ni oscuro.`;

const TEMPLATE_PUBLICO_OBJETIVO = `Usa la imagen adjunta del producto EXACTAMENTE como está, sin modificarla. Solo se compone sobre el fondo nuevo.

Genera una infografía cuadrada de 1200x1200 px para Mercado Libre que comunique para quién es ideal {NOMBRE_PRODUCTO}, SIN mostrar personas ni rostros generados — todo el mensaje se transmite con texto, íconos y el producto, nunca con figuras humanas.

COMPOSICIÓN:
- Título superior extra bold en 2 líneas: línea 1 en {COLOR_TEXTO} "{FRASE_GANCHO_LINEA_1}", línea 2 en {COLOR_ACENTO} más grande "{FRASE_GANCHO_LINEA_2}".
- Un ícono grande y representativo del público objetivo (ej. una figura humana simplificada tipo pictograma/silueta plana de ícono, nunca un rostro realista ni una foto de una persona) en {COLOR_ACENTO}.
- El producto (imagen subida) al costado, tamaño mediano (40-50% del alto), sombra realista.
- Fondo en {COLOR_PRIMARIO} con formas geométricas desenfocadas en {COLOR_ACENTO}.

BARRA INFERIOR: 2-3 razones con ícono, mismo estilo lineal, texto en {COLOR_TEXTO}: "{BENEFICIO_1}", "{BENEFICIO_2}", "{BENEFICIO_3}".

ESTILO: sigue el SISTEMA DE DISEÑO.

NO HACER: no generar personas, rostros, manos ni cuerpos humanos fotorrealistas de ningún tipo — el público objetivo se representa solo con pictogramas/íconos planos, nunca con figuras realistas. No usar fondo negro ni oscuro.`;

const TEMPLATE_VERSATILIDAD = `Usa la imagen adjunta del producto EXACTAMENTE como está, sin modificarla. Solo se compone sobre el fondo nuevo.

Genera una infografía cuadrada de 1200x1200 px para Mercado Libre que muestre las distintas formas reales de usar {NOMBRE_PRODUCTO}.

COMPOSICIÓN:
- Título superior extra bold: "{FRASE_GANCHO_LINEA_1}" en {COLOR_TEXTO}, "{FRASE_GANCHO_LINEA_2}" en {COLOR_ACENTO} debajo, más grande.
- 2-3 tarjetas en {COLOR_PANEL} con esquinas redondeadas en fila, cada una con un ícono distinto representando una forma de uso + texto corto en {COLOR_TEXTO}: "{BENEFICIO_1}", "{BENEFICIO_2}", "{BENEFICIO_3}".
- El producto (imagen subida) centrado o al costado, tamaño mediano (40-50% del alto), sombra realista.
- Fondo en {COLOR_PRIMARIO} con formas geométricas desenfocadas en {COLOR_ACENTO}.

ESTILO: sigue el SISTEMA DE DISEÑO. Sensación de flexibilidad y variedad.

NO HACER: no inventar formas de uso que no estén en el texto real. No usar fondo negro ni oscuro.`;

const TEMPLATE_ETIQUETA = `Usa la imagen adjunta del producto EXACTAMENTE como está, sin modificarla. Solo se compone sobre el fondo nuevo.

Genera una infografía cuadrada de 1200x1200 px para Mercado Libre que presente de forma clara la información real del empaque/etiqueta de {NOMBRE_PRODUCTO}.

COMPOSICIÓN:
- Título superior: "{FRASE_GANCHO_LINEA_1}" en {COLOR_TEXTO}.
- El producto (imagen subida) grande, tamaño protagonista (55-65% del alto), de forma que la etiqueta/información real impresa en el envase se lea con claridad — no lo tapes con texto ni íconos.
- 2-3 datos reales del empaque destacados en tarjetas {COLOR_PANEL} alrededor (no sobre el envase), texto en {COLOR_TEXTO}: "{BENEFICIO_1}", "{BENEFICIO_2}", "{BENEFICIO_3}".
- Fondo en {COLOR_PRIMARIO}, simple, para no competir con la información real del envase.

ESTILO: sigue el SISTEMA DE DISEÑO, pero priorizando que la etiqueta real del producto sea perfectamente legible por sobre la decoración.

NO HACER: no inventar datos del empaque (peso, contenido, ingredientes) que no estén en el texto real ni tapar la información impresa real del envase. No usar fondo negro ni oscuro.`;
...
const TEMPLATES: Record<InfographicSetCategory, string> = {
  ...
  rendimiento: TEMPLATE_RENDIMIENTO,
  instrucciones: TEMPLATE_INSTRUCCIONES,
  confianza: TEMPLATE_CONFIANZA,
  producto_limpio: TEMPLATE_PRODUCTO_LIMPIO,
  composicion: TEMPLATE_COMPOSICION,
  publico_objetivo: TEMPLATE_PUBLICO_OBJETIVO,
  versatilidad: TEMPLATE_VERSATILIDAD,
  etiqueta: TEMPLATE_ETIQUETA,
};
